...
def Loss(img_shape, feat_shape, max_fg_num = 128, max_bg_num = 128, rpn_neg_thres = 0.3, rpn_pos_thres = 0.7):
  ...
  # find a anchor box with best iou for every ground truth box
  ...
  best_anchor = tf.keras.layers.Lambda(lambda x: tf.math.argmax(x, axis = 0, output_type = tf.int32))(iou); # best_anchor.shape = (m,)
  best_iou = tf.keras.layers.Lambda(lambda x: tf.math.reduce_max(x, axis = 0))(iou); # best_iou.shape = (m)
  labels = tf.keras.layers.Lambda(lambda x, nt, pt: tf.where(tf.math.less(x, nt), tf.zeros_like(x), tf.where(tf.math.greater(x,pt), tf.ones_like(x), -tf.ones_like(x))), 
                                  arguments = {'nt': rpn_neg_thres, 'pt': rpn_pos_thres})(best_iou); # labels.shape = (m,)
  ...
  labels = tf.keras.layers.Lambda(lambda x, n: tf.cond(tf.math.less(x[0], n), 
                                  ...
                                  arguments = {'n': max_fg_num})([count, labels]); # labels.shape = (m,)
  ...
  labels = tf.keras.layers.Lambda(lambda x, n: tf.cond(tf.math.less(x[0], n),
                                  ...
                                  arguments = {'n': max_bg_num})([count, labels]); # labels.shape = (m,)
  
  
  return tf.keras.Model(inputs = (bbox_pred, cls_pred, gt_bbox), outputs = labels);
