...
def CTPN(input_shape, hidden_units = 128, output_units = 512):
  ...
  bbox_pred = tf.keras.layers.Reshape((bbox_pred.shape[-3], bbox_pred.shape[-2], 10, 6))(bbox_pred); # bbox_pred.shape = (batch, h, w, anchor_num = 10, 6) in sequence of (dx dy dw dh logits0 logits1)
...
def SampleLoss(feat_shape, max_fg_anchors = 128, max_bg_anchors = 128, rpn_neg_thres = 0.3, rpn_pos_thres = 0.7):
  ...
  anchors_wh = tf.keras.layers.Lambda(lambda x: x[...,2:4] - x[...,0:2] + 1)(anchors); # anchors_wh.shape = (h, w, 10, 1, 2)
  ...
  best_gt = tf.keras.layers.Lambda(lambda x: tf.gather(x[1], x[0]))([best_gt_idx, gt_bbox]); # best_gt.shape = (h, w, 10, 5)
